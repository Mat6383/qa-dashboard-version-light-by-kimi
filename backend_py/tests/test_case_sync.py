...
def test_build_case_payload_truncates_description() -> None:
    ...
    assert payload["custom_description"].endswith("…")
    assert len(payload["custom_description"]) == 2001
...
async def test_sync_iteration_not_found(svc: CaseSyncService) -> None:
    with patch("app.services.case_sync.gitlab_service.find_iteration_for_project", new=AsyncMock(return_value=None)):
...
async def test_sync_iteration_creates_new_case(svc: CaseSyncService) -> None:
    with patch("app.services.case_sync.gitlab_service.find_iteration_for_project", new_callable=AsyncMock) as mock_iter, \
         ...
         patch("app.services.case_sync.gitlab_service.update_issue_label", new_callable=AsyncMock) as mock_label, \
         patch("app.services.case_sync.testmo_service.get_or_create_folder", new_callable=AsyncMock) as mock_folder, \
         patch("app.services.case_sync.testmo_service.get_cases", new_callable=AsyncMock) as mock_cases, \
    ...
    mock_label.assert_called_once_with(1, 1, add_labels=["Sync-Updated"], remove_labels=[])
...
async def test_sync_iteration_updates_existing_case(svc: CaseSyncService) -> None:
    with patch("app.services.case_sync.gitlab_service.find_iteration_for_project", new_callable=AsyncMock) as mock_iter, \
         ...
         patch("app.services.case_sync.testmo_service.get_or_create_folder", new_callable=AsyncMock) as mock_folder, \
         patch("app.services.case_sync.testmo_service.get_cases", new_callable=AsyncMock) as mock_cases, \
         patch("app.services.case_sync.testmo_service.update_case", new_callable=AsyncMock) as mock_update:
        mock_iter.return_value = {"id": 10, "title": "R06"}
...
async def test_sync_iteration_skips_enriched(svc: CaseSyncService) -> None:
    with patch("app.services.case_sync.gitlab_service.find_iteration_for_project", new_callable=AsyncMock) as mock_iter, \
        ...
        mock_folder.return_value = {"id": 99}
        mock_cases.return_value = [{"id": 50, "name": "Rich case", "estimate": 3600, "issues": [{"display_id": "1"}]}]
